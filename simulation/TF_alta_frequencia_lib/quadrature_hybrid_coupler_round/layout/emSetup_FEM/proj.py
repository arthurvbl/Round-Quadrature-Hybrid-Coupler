...
def set_frequency_plan_and_common_options():
	...
	frequency_plan.startFrequency=750000000
	frequency_plan.stopFrequency=3750000000
	frequency_plan.samplePointsLimit=50
	frequency_plan_list.append(frequency_plan)
	if 'minFreq' in empro.activeProject.parameters:
		empro.activeProject.parameters.setFormula('minFreq','0.75 GHz')
	if 'maxFreq' in empro.activeProject.parameters:
		empro.activeProject.parameters.setFormula('maxFreq','3.75 GHz')
...
def get_session(usedFlow="ADS"):
	...
	if ads_import_version >= 3:
		session=empro.toolkit.ads_import.Import_session(units="mil", wall_boundary="Radiation",usedFlow=usedFlow,adsProjVersion=getVersion())
		return session
	try:
		session=empro.toolkit.ads_import.Import_session(units="mil", wall_boundary="Radiation",usedFlow=usedFlow)
...
class projImporter():
	...
	def create_materials(self,materialForEachLayer=False):
		...
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
			material=self.session.create_material(name=EMProMaterialName, color=(255,0,0,255), thickness="3.5e-05 m", resistance=0)
			try:
		...
		ADSmaterialName=["RO4003C","__SubstrateLayer1"][materialForEachLayer]
		extraMaterialProperties=(50,None,None,False) # (priority,thickness,modelTypeForMetals,convertedToResistance)
		material=ADSmaterialMap.get((ADSmaterialName,extraMaterialProperties),None)
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
			material=self.session.create_material(name=EMProMaterialName, color=(202,225,255,128), permittivity=3.55, losstangent=0.0021, permeability=1, use_djordjevic=True, lowfreq=1000, evalfreq=1000000000, highfreq=1000000000000)
			try:
	...
	def initNetlists(self):
		netlistNames = ['net_0_P1_P2_P3_P4','net_1']
	...
	def ads_import(self,usedFlow="ADS",topAssembly=None,demoMode=False,includeInvalidPorts=True,suppressNotification=False,materialForEachLayer=False):
		...
		param_list.setFormula( "lateralExtension", "0 mil")
		param_list.setFormula( "verticalExtension", "0 mil")
		...
		param_list.setFormula( "lateralExtension", "123.031496062992 mil")
		param_list.setFormula("verticalExtension", "196.850393700787 mil")
	...
	def create_geometry(self,topAssembly):
		...
		assembly=empro.geometry.Assembly()
		part=empro.geometry.Model()
		simBox = empro.geometry.Box( _createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox-xLowerBoundingBox", "abs((-0.0189037976-xLowerExtension)-(0.0219915994+xUpperExtension))"), _createIfToggleExtensionToBoundingBoxExpression("zUpperBoundingBox-zLowerBoundingBox", "((((stack_RO4003C_layer_3_Z) + (zUpperExtension)) - (stack_RO4003C_layer_1_Z)))"), _createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox-yLowerBoundingBox" , " abs((-0.031989649-yLowerExtension)-(0.008905748+yUpperExtension))"))
		part.recipe.append(simBox)
		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(_createIfToggleExtensionToBoundingBoxExpression("(xUpperBoundingBox+xLowerBoundingBox)/2", "(0.0219915994+xUpperExtension+-0.0189037976-xLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("(yUpperBoundingBox+yLowerBoundingBox)/2", "(0.008905748+yUpperExtension+-0.031989649-yLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("zLowerBoundingBox","((stack_RO4003C_layer_1_Z) - (0))")))
		...
		self.session.adjust_view()
		assembly=empro.geometry.Assembly()
		pointString='0.0219915994+xUpperExtension#-0.031989649-yLowerExtension;0.0219915994+xUpperExtension#0.008905748+yUpperExtension;-0.0189037976-xLowerExtension#0.008905748+yUpperExtension;-0.0189037976-xLowerExtension#-0.031989649-yLowerExtension'
		sketch = self._create_sketch(pointString)
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex0",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","-0.0189037976-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.031989649-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex1",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.0219915994+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.031989649-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex2",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.0219915994+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox","0.008905748+yUpperExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex3",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","-0.0189037976-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox","0.008905748+yUpperExtension"),0)))
		part=empro.geometry.Model()
		part.recipe.append(empro.geometry.Extrude(sketch,"(stack_RO4003C_layer_3_Z) - (stack_RO4003C_layer_1_Z)",V(0,0,1)))
		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(0,0,"(stack_RO4003C_layer_1_Z) - (0)"))
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['-0.00913003#-0.0183572912;-0.0052714144#-0.0222159068;-3.1496e-06#-0.0236384084;0.000999998#-0.0236277912;0.000999998#-0.031989649;0.0020877784#-0.031989649;0.0020877784#-0.0236277912;0.0030877764#-0.0240122202;0.0085509608#-0.022548342;0.0125502924#-0.0185490358;0.0140141452#-0.0130858514;0.0136297416#-0.012085828;0.0219915994#-0.012085828;0.0219915994#-0.0109980476;0.0136297416#-0.0109980476;0.0136403588#-0.0099949;0.0122178572#-0.0047266352;0.0083592162#-0.0008679942;0.0030909514#0.0005545074;0.0020878038#0.0005438902;0.0020878038#0.008905748;0.0010000234#0.008905748;0.0010000234#0.0005438902;0#0.0009282938;-0.0054631844#-0.000535559;-0.0094624906#-0.0045348906;-0.0109263688#-0.009998075;-0.0105419398#-0.0109980476;-0.0189037976#-0.0109980476;-0.0189037976#-0.012085828;-0.0105419398#-0.012085828;-0.010552557#-0.0130890264']
		pointStrings.append('-0.0094541594#-0.0109980476;-0.0090697558#-0.009998075;-0.0078546452#-0.0054631844;-0.0045348906#-0.0021434044;0#-0.0009282938;0.000999998#-0.0005438902;0.0030877764#-0.0005438902;0.0078126336#-0.0018126202;0.0112732312#-0.0052732178;0.0125419612#-0.009998075;0.0125419612#-0.0120858534;0.0121575576#-0.0130858514;0.010942447#-0.017620742;0.007622667#-0.0209404966;0.0030877764#-0.0221556072;0.0020878038#-0.0225400108;2.54e-08#-0.0225400108;-0.0047248064#-0.0212712808;-0.0081854294#-0.0178106832;-0.0094541594#-0.0130858514')
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['0.0251165994#-0.035114649;0.0251165994#0.012030748;-0.0220287976#0.012030748;-0.0220287976#-0.035114649']
		sketch = None
		for pointString in pointStrings:
			sketch = self._create_sketch(pointString, sketch)
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex0",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","-0.0189037976-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.031989649-yLowerExtension"),0)))
		...
		try:
			...
			if mtrl.details.materialType() == "Physical": 
				...
				if eProp.propertyType() == "PEC" or eProp.parameters.parametersType() == "PEC":
					...
					if (float(empro.core.Expression("((stack_RO4003C_layer_1_Z) - (0))")) <= bbox_3dcs.lower.z + SPAresabs ) :
						part.meshParameters.includeInMesh=False
						empro.activeProject.parameters.setFormula( "zLowerExtension", "0 mil")
		except AttributeError:
			part.meshParameters.includeInMesh=False
	...
	def getMaskHeights(self,parameterized=False):
		...
		mask_heights[2]=(0.000508, 0.000508)
	...
	def create_ports( self, topAssembly, includeInvalidPorts=True, symbPinData=None):
		...
		head=V("(-0.0189037976) - (0)","(-0.0115419378) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(-0.0189037976) - (0)","(-0.0115419378) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(-0.0189037976) - (0)","(-0.012085828) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent.endPoint2Position=V("(-0.0189037976) - (0)","(-0.0109980476) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		...
		head=V("(0.0015439136) - (0)","(0.008905748) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(0.0015439136) - (0)","(0.008905748) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0.0010000234) - (0)","(0.008905748) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent.endPoint2Position=V("(0.0020878038) - (0)","(0.008905748) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		...
		for headOrTail in headsAndTails:
			...
			if abs(float(headOrTail.z) - zLowerBoundary) < SPAresabs: internalPortOnZLowerBoundary = True
			if abs(float(headOrTail.z) - zUpperBoundary) < SPAresabs: internalPortOnZUpperBoundary = True
		head=V("(0.0219915994) - (0)","(-0.0115419378) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(0.0219915994) - (0)","(-0.0115419378) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0.0219915994) - (0)","(-0.012085828) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent.endPoint2Position=V("(0.0219915994) - (0)","(-0.0109980476) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		...
		for headOrTail in headsAndTails:
			...
			if abs(float(headOrTail.z) - zLowerBoundary) < SPAresabs: internalPortOnZLowerBoundary = True
			if abs(float(headOrTail.z) - zUpperBoundary) < SPAresabs: internalPortOnZUpperBoundary = True
		head=V("(0.0015438882) - (0)","(-0.031989649) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(0.0015438882) - (0)","(-0.031989649) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0.000999998) - (0)","(-0.031989649) - (0)","(stack_RO4003C_layer_1_Z) - (0)")
		extent.endPoint2Position=V("(0.0020877784) - (0)","(-0.031989649) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
	...
	def create_parameters(self):
		self._create_parameter("stack_RO4003C_layer_1_Z", "0 mil", "Z of topology level (level 1 of stack RO4003C)",True,fixGridAxis='Z')
		self._create_parameter("stack_RO4003C_layer_3_Z", "20 mil", "Z of topology level (level 3 of stack RO4003C)",True,fixGridAxis='Z')
		self._create_parameter("lateralExtension","123.031496062992 mil","Substrate LATERAL extension", True)
		self._create_parameter("verticalExtension","196.850393700787 mil","Substrate VERTICAL extension", True)
		...
		self._create_parameter("mask_cond_Zmin",str("20 mil"),"Zmin of mask cond",True,fixGridAxis='Z')
