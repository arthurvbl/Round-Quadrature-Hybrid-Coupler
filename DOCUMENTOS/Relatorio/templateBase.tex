...
\label{sec:teoria_impedancia}

Antes da análise dos resultados obtidos, é necessário entender primeiramente a teoria por trás do funcionamento do acoplador híbrido. O circuito baseia-se no princípio da superposição geométrica e elétrica, o qual é resolvido através da Análise de Modos Par e Ímpar. Devido à simetria perfeita da topologia de quatro portas, a estrutura pode ser dividida ao longo de seu eixo de simetria vertical. A excitação das portas é então decomposta em dois modos distintos, permitindo a análise de um "meio-circuito" simplificado.

No plano de simetria, as linhas de transmissão horizontais (em série) permanecem intactas, enquanto as linhas verticais (em paralelo), originalmente com comprimento elétrico de $\lambda/4$, são seccionadas ao meio, resultando em \textit{stubs} de comprimento $\lambda/8$.

\subsubsection{Análise de modo par}
No modo par, excitações de mesma amplitude e fase são aplicadas simultaneamente às portas simétricas, como mostra a \cref{fig:even_mode}. Isso força a corrente no eixo de simetria a ser nula, criando um circuito aberto virtual.

\begin{figure}[H]
\begin{center}
    \includegraphics[width=0.45\textwidth]{even_mode}
\end{center}
\caption{Acoplador em modo par.}\label{fig:even_mode}
\end{figure}

A admitância de entrada de um \textit{stub} em aberto de comprimento $\lambda/8$ ($45^{\circ}$) é dada por $Y_{in} = j Y_0 \tan(45^{\circ})$. Logo, a admitância dos \textit{stubs} paralelos no modo par assume o valor:

\begin{equation}
    Y_{par} = j Y_p
\end{equation}

\subsubsection{Análise de modo ímpar}
No modo ímpar, aplicam-se excitações com defasagem de $180^{\circ}$, como é possível identificar na \cref{fig:odd_mode}. A tensão no plano de simetria torna-se identicamente nula, criando um curto-circuito virtual para o terra. 

\begin{figure}[H]
\begin{center}
    \includegraphics[width=0.45\textwidth]{odd_mode}
\end{center}
\caption{Acoplador em modo ímpar.}\label{fig:odd_mode}
\end{figure}

A admitância de um \textit{stub} em curto de comprimento $\lambda/8$ é $Y_{in} = -j Y_0 \cot(45^{\circ})$, resultando em:

\begin{equation}
    Y_{\text{ímpar}} = -j Y_p
\end{equation}

\subsubsection{Matriz de Transmissão}

O meio-circuito é modelado matematicamente por uma Matriz de Transmissão (Matriz ABCD), consistindo na cascata de três elementos: o \textit{stub} paralelo de entrada, a linha de transmissão em série de $\lambda/4$ (com admitância $Y_s$) e o \textit{stub} paralelo de saída. A matriz equivalente do modo par $[M_e]$ é obtida pelo produto:

\begin{equation}
    [M_e] = \begin{bmatrix} 1 & 0 \\ jY_p & 1 \end{bmatrix} \begin{bmatrix} 0 & j/Y_s \\ jY_s & 0 \end{bmatrix} \begin{bmatrix} 1 & 0 \\ jY_p & 1 \end{bmatrix}
\end{equation}

Realizando a multiplicação matricial, obtém-se:
\begin{equation}
    [M_e] = \begin{bmatrix} -\frac{Y_p}{Y_s} & j\frac{1}{Y_s} \\ j\frac{Y_s^2 - Y_p^2}{Y_s} & -\frac{Y_p}{Y_s} \end{bmatrix}
\end{equation}

Para que o acoplador opere com casamento de impedância ideal na porta de entrada ($S_{11} = 0$), é necessário que o elemento $B$ da matriz ABCD normalizada seja igual ao elemento $C$. Igualando os termos correspondentes da matriz $[M_e]$:

\begin{equation}
    j\frac{1}{Y_s} = j\frac{Y_s^2 - Y_p^2}{Y_s}
\end{equation}

Multiplicando ambos os lados por $Y_s / j$, chega-se à relação geral de admitância: 
\begin{equation}
    Y_s^2 - Y_p^2 = 1 
    \label{eq:admitancia_geral}
\end{equation}

Considerando a impedância $Z = 1/Y$, chega-se à relação geral de impedância: 

\begin{equation}
    \left( \frac{Z_0}{Z_s} \right)^2 - \left( \frac{Z_0}{Z_p} \right)^2 = 1
    \label{eq:impedancia_geral}
\end{equation}

Para a configuração de um híbrido de 3 dB, que garante uma divisão equitativa de potência entre a porta direta e a porta acoplada, devemos considerar que as linhas paralelas possuam a mesma impedância característica do sistema, ou seja, $Z_p = Z_0$. Considerando a impedância de referência $Z_0 = 50 \, \Omega$, substitui-se o valor na Equação \ref{eq:impedancia_geral}:

\begin{equation}
    \left( \frac{50}{Z_s} \right)^2 - 1 = 1 \implies \left( \frac{50}{Z_s} \right)^2 = 2
\end{equation}

Isolando a impedância $Z_s$, obtém-se o valor analítico para a linha em série:
